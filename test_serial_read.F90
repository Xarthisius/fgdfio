program test
  ...
  !character(len=20), dimension(:), pointer  :: adata
  integer(kind=4), dimension(:), pointer :: adata
  ...
  call read_int_attribute(g_id, "domain_dimensions", len, adata)
...
contains
  ...
  subroutine read_int_attribute(g_id, name, len, int_array)
    ...
    integer(HSIZE_T), dimension(1), intent(in) :: len
    ...
    call h5screate_simple_f(I_ONE, len, aspace_id, error)
    call h5aopen_f(g_id, name, attr_id, error)

    allocate(adata(1:len(1)))
